package cc.aisc.core.config.shiro.realm;
...
//            User user = userService.findByUsername(loginName).get();
            ...
            info.setRoles(userService.findRoles(username).get());
            info.setStringPermissions(userService.findPermissions(username).get());
            //用户的角色集合
//            info.setRoles(userService.getRoles(user));
            ...
            logger.warn("no record by username = {}", username);
        }
    ...
    protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken authenticationToken) throws AuthenticationException {
        ...
        try {
            ...
            return new SimpleAuthenticationInfo(
                    ...
                    user.getPassword(), //密码
                    ByteSource.Util.bytes(user.getPassword()),//salt=username+salt //todo
                    getName() //realm name
            );
        }catch (NoSuchElementException e){
